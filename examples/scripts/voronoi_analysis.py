...
and lists the 10 most frequent polyhedron types in the glass. In the case
...
# Import a simulation of Cu-Zr metallic glass.
node = import_file("../data/CuZr_metallic_glass.dump.gz")

# Set atomic radii (required for radical Voronoi tessellation).
...
# This will be an (N)x(edge_count) array.
voro_indices = node.output["Voronoi Index"].array

# This helper function takes a two-dimensional array and computes a frequency histogram of the data rows 
# using some NumPy magic. It returns two arrays (of equal length): 
